...
[`albersdown::use_albers_vignettes()`](https://bbuchsbaum.github.io/albersdown/reference/use_albers_vignettes.md)).
...
- Vignettes: `getting-started`, `design-notes`, `theme-lab`,
  `theme-showcase`, `proof-teal-study`, `proof-ochre-structural`.
- Theme Lab article: interactive family/preset/style/content-width
  preview.
- Theme Showcase article: dark preset + non-red accent family gallery.
- Proof articles: full-page fixed combinations for `teal + study` and
  `ochre + structural`.
...
- Visual regression scaffold: `tests/visual-regression/` plus workflow
...
- In YAML, add: `params: { family: "red", preset: "study" }`
- In setup, call:
...
- Add `albers-header.html` alongside `albers.css` and `albers.js`, then
  point `includes: in_header` at it.
